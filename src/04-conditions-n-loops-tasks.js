...
function findFirstSingleChar(str) {
  ...
  const firstNonRepeated = charArray.find((char) => charCount[char] === 1);
  return firstNonRepeated || null;
}
...
function getDigitalRoot(num) {
  let currentNum = num;
  while (currentNum > 9) {
    currentNum = currentNum.toString().split('').reduce((acc, digit) => acc + Number(digit), 0);
  }
  return currentNum;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  function findCommonComponents(index) {
    const currentComponent = splitPaths[0][index];
    if (index >= splitPaths[0].length) {
      return '';
    }
    if (splitPaths.every((pathComponents) => pathComponents[index] === currentComponent)) {
      return `${currentComponent}/${findCommonComponents(index + 1)}`;
    }
    return '';
  }
  const commonPath = findCommonComponents(0);
...
function getMatrixProduct(m1, m2) {
  ...
  const dotProduct = (row, colIndex) => row
    .reduce((sum, value, rowIndex) => sum + value * m2[rowIndex][colIndex], 0);
  return m1.map((row) => Array
    .from({ length: colsM2 }, (_, colIndex) => dotProduct(row, colIndex)));
}
...
function evaluateTicTacToePosition(position) {
  ...
  const winner = winningCombinations
    .find((combination) => combination
      .every((value) => value === 'X') || combination.every((value) => value === '0'));
